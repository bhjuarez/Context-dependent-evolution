...
library(ape); library(geiger); library(geomorph); library(phytools); library(dichromat); library(ggplot2)

#setwd("/Users/Bryan/Dropbox/cods")
setwd("")
tree = read.nexus("data/dat.tree.nex")
alldat = read.csv("data/dat.csv")
attach(alldat)
...
### plot length ~ depth
pdf("Fig1_length.pdf", width = 4, height = 5)
par(mfrow = c(2, 1))
keep = complete.cases(length, depth)
dat = data.frame(length = length[keep], depth = depth[keep], pelagic = pelagic[keep])
rownames(dat) = taxon[keep]
...
plot(log(treedat$data[, 2]), log(treedat$data[, 1]), 
	...
	xlab = "", ylab = "") #plot data
clip(0, log(200), 0, log(100))
abline(pgls.1.coeff[[2]], lwd = 2) #plot slopes
clip(log(200), log(10000), 0, log(100))
...
legend("bottomright", legend = c("Photic", "Disphotic"), pch = c(16, 1), lty = c(1, 2), cex = 0.5, box.col = "gray")
...
ggfactor[ggfactor == 1] <- "Disphotic"
ggdat <- data.frame(diam = treedat$data[, 1], Zone = as.factor(ggfactor))
levels(ggdat$Zone) <- c("Photic", "Disphotic")

linesdat <- data.frame(Zone = c("Photic", "Disphotic"), medians = tapply(ggdat$diam, ggdat$Zone, median))

pdf("Fig2_diam.pdf", width = 4, height = 3)

ggplot(ggdat, aes(x = diam, col = Zone)) + 
    geom_density() +
    geom_vline(data = linesdat, aes(xintercept = medians, color = Zone), linetype = "dashed") + 
    labs(x = expression(paste("Ommatidium Diameter (", mu, "m)")), y = "Density") +
    scale_color_grey() + 
    theme_classic()
...
pdf("Fig3_count.pdf", width = 5, height = 5)
...
legend("topleft", legend = c("Photic", "Disphotic"), pch = c(16, 1), lty = c(1, 2), cex = 0.9, box.col = "gray")
...
pdf("Fig4_phylomorphospace.pdf", width = 6, height = 6)
...
pdf("Fig5_ace_color.pdf", width = 4, height = 4)
...
spp = gsub(" J57062", "", spp)
text(rep(0, length(drop$tip.label)), 1:length(drop$tip.label), spp, cex = 0.85, font = c(rep(3, 43), rep(4, 13), rep(3, 35)), col = c(rep("black", 43), rep("red", 13), rep("black", 35)))
...
levels(resample3$pelagic) <- c("Photic", "Disphotic", "Aphotic")

vline.data <- data.frame(z = c(rats), pelagic = c("Photic", "Disphotic", "Aphotic"))
dat_text <- data.frame(
  label = c("*", "", "*"),
  pelagic   = c("Photic", "Disphotic", "Aphotic"),
  x     = c(0.3, 0, 0.3),
  y     = c(6.5, 0, 4)
)

pdf("Fig6_permutation_test.pdf", width = 6, height = 6)

ggplot(resample3, aes(x = ratios)) + geom_density(fill = "gray", alpha = 0.5, bw = 0.05) + facet_grid(pelagic~.) + geom_vline(aes(xintercept = z), vline.data, lwd = 1.2) + labs(x = "Ratio of Eyeless:Eyed Species", y = "Density") + theme_classic() + geom_text(data = dat_text, mapping = aes(x = x, y = y, label = label), size = 10)
