...
pdf("rel_eyelength.pdf", width = 4, height = 4)
par(mar = c(5, 4, 4, 2) + 0.1 + c(-0.5, 0.5, 0, 0))
plot(log(treedat$data[, 3]), log(treedat$data[, 2] / treedat$data[, 1] / 1000), 
	xaxt = "n", yaxt = "n", 
	pch = c(16, 1)[as.factor(treedat$data[, 4])], 
	xlab = "", ylab = "") #plot data
abline(pgls.2.coeff[[2]], lwd = 2) #plot slopes
abline(pgls.2.coeff[[3]], lwd = 2, lty = 2)
par(new = T)
plot(treedat$data[, 3], treedat$data[, 2] / treedat$data[, 1] / 1000, 
	log = "xy", type = "n", 
	xlab = "Depth (m)", ylab = "Relative Eye Length (mm/mm)", cex.axis = 0.7, cex.lab = 1) #plot log-scale axes
legend("bottomleft", legend = c("Photic", "Dysphotic"), pch = c(16, 1), lty = c(1, 2), cex = 0.85, box.col = "gray")
dev.off()
