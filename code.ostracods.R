...
legend("topleft", legend = c("Photic", "Dysphotic"), pch = c(16, 1), lty = c(1, 2))
...
plot(treedat$data[, 2], treedat$data[, 1], 
	log = "xy", type = "n", 
	xlab = "Depth (m)", ylab = "Length (mm)", cex.lab = 1.25) #plot log-scale axes
legend("topleft", legend = c("Eyed", "Eyeless"), pch = c(16, 1), lty = c(1, 2))
...
## count ~ depth
keep1 = complete.cases(count, depth)
dat = cbind(count[keep1], depth[keep1])
keep2 = apply(dat, 1, function(row) {all(row != 0 )})
dat = dat[keep2,]
dat = data.frame(count = dat[, 1], depth = dat[, 2], pelagic = pelagic[keep1][keep2])
rownames(dat) = taxon[keep1][keep2]
dat = dat[which(dat[, 3] != 2),] #clean data, remove aphotic zone species
...
legend("topleft", legend = c("Euphotic", "Dysphotic"), pch = c(16, 1), lty = c(1, 2))
...
dat = alldat[, c("count", "pelagic")]
dat = dat[which(!is.na(dat$count)),]
attach(dat)

rats = c(length(count[(count == 0 & pelagic == 0)]) / length(which(pelagic == 0)),
         length(count[(count == 0 & pelagic == 1)]) / length(which(pelagic == 1)), 
         length(count[(count == 0 & pelagic == 2)]) / length(which(pelagic == 2))) 

reps = 99999
sample.n = c(dim(dat[pelagic == 0,])[[1]], dim(dat[pelagic == 1,])[[1]], dim(dat[pelagic == 2,])[[1]])
n = sum(sample.n)
resample = array(dim = c(reps, 3))

for(i in 0:2){
	for(j in 1:reps){
		boot = sample(n, sample.n[[i + 1]])
		resample[j, i + 1] = with(dat[boot,], length(which(count == 0)) / dim(dat[boot,])[[1]])
	}
}

upper = apply(resample, 2, function(x) {quantile(x, 0.95)})
lower = apply(resample, 2, function(x) {quantile(x, 0.05)})

1 - length(which(rats[[1]] < resample[, 1])) / (reps + 1)
...
axis(side = 1, at = c(0, 0.5, 1), labels = c("Euphotic", "Dysphotic", "Aphotic"), cex.axis = 1.25)
